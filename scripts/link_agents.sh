...
#
# Usage:
#   ./scripts/link.sh                # init all supported IDEs
#   ./scripts/link.sh --ide cursor   # init a single IDE
#   ./scripts/link.sh --check        # dry-run: report issues, exit non-zero
#   ./scripts/link.sh --clean        # remove all managed symlinks
#   ./scripts/link.sh --log-level verbose
#   ./scripts/link.sh -h | --help
# =============================================================================
...
if $CHECK_ONLY; then
    ...
    if [[ $CHECK_FAILED -gt 0 ]]; then
        log_error "CHECK mode: $CHECK_FAILED issue(s) found. Run link.sh to fix."
        exit 1
    fi
else
    ...
    log_info ""
    log_info "Re-run scripts/link.sh after modifying anything under .agents/."
