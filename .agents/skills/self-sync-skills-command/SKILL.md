...
5. After all commands are in place, run `./scripts/link_agents.sh` to propagate symlinks to IDE directories.
...
- The relative path `../skills/<name>/SKILL.md` works uniformly from every IDE `commands/` directory after `link_agents.sh` creates the per-item symlinks.
...
- After generating or updating commands, always run `./scripts/link_agents.sh` so IDE directories pick up the changes.
