...
> [!cite] > [HTTP的三种身份认证：基本(Basic)认证、摘要(Digest)认证、基于表单(Form-Based)的SSL客户端认证 | 霍小强博客](https://www.huoxiaoqiang.com/experience/httpe/6451.html)
...
<Badge type="warning">ttttttttttt</Badge>

- [[../计算机网络-HTTP长连接和短连接|计算机网络-HTTP长连接和短连接]]
- 本质上就是**允许单个 `TCP` 连接上发送多个 `HTTP` 请求和响应，不必每次请求都单独创建新的连接。**
- `HTTP 1.1` 默认采取持久连接的方式。`HTTP1.0` 需要显式指定 `Connetction: keep-alive` 才能启用持久连接，但是 `HTTP1.1` 默认支持。_不必在头部指定，1.1 默认开启_。
