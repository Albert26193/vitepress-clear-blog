...
test.describe('D3HomePage', () => {
  test('homepage graph container renders', async ({ page }) => {
    await page.goto('/')
    const container = page.locator('.d3-overall-container')
    await expect(container).toBeVisible({ timeout: 15000 })
  })

  test('graph has SVG with nodes', async ({ page }) => {
    await page.goto('/')
    await page.waitForSelector('.d3-overall-container svg circle', {
      timeout: 15000
    })
    const circles = page.locator('.d3-overall-container svg circle')
  ...
  test('graph nodes are labeled', async ({ page }) => {
    ...
    const texts = page.locator('.d3-overall-container svg text')
